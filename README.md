...
So an agent/tool running **inside** the container (e.g. Grok) can `Ctrl+V` a host screenshot — `image/png`, which OSC 52 can't carry — the wizard bind-mounts the host compositor's **Wayland + D-Bus sockets** into `/tmp/host-wayland/` and sets `WAYLAND_DISPLAY` (absolute) + `DBUS_SESSION_BUS_ADDRESS` in `containerEnv`. `wl-clipboard` is baked into the image. `--security-opt=label=disable` lets the container open the sockets under SELinux (same relaxation used for accel devices). No `--userns` needed — DevPod's podman provider already maps host uid 1000 → container `vscode`, so the socket is owned by `vscode` inside.
